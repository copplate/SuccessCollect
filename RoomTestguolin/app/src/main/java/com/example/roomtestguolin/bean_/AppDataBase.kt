...
@Database(entities = [User::class,WatchHis::class], version = 2)
...
//                                "  (`id` INTEGER PRIMARY KEY AUTOINCREMENT,'uid' text not null,url text not null);")
                        database.execSQL("CREATE TABLE IF NOT EXISTS `WatchHis` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `uid` TEXT NOT NULL, `url` TEXT NOT NULL)")
