...
public class ShortEditActivity extends AppCompatActivity {
    ...
    private int CHOOSE_CODE = 9; // 选择视频封面的请求码
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        compressVideoPath = getIntent().getStringExtra("compress_video_path");
        initView(); // 初始化视图
    ...
    private void initView() {
        ...
        findViewById(R.id.btn_publish).setOnClickListener(v -> myShortPublish());
//        findViewById(R.id.btn_publish).setOnClickListener(v -> shortPublish());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (resultCode == RESULT_OK && requestCode == CHOOSE_CODE) {
            mCoverPos = intent.getIntExtra("cover_pos", 0);
            new Thread(() -> showCover()).start(); // 启动线程展示封面图片
        }
    }
    ...
    private void showCover() {
        // 获取视频文件中的某帧图片
        Log.d("tiktok", "showCover: -----mVideoPath----" + mVideoPath);
    ...
    private void myShortPublish() {
        ...
        // 往建造器对象添加文本格式的分段数据
        builder.addFormDataPart("userId", "555"); // 拍摄日期

        // 往建造器对象添加视频格式的分段数据
        /*builder.addFormDataPart("video", mVideoPath.substring(mVideoPath.lastIndexOf("/")),
                RequestBody.create(new File(mVideoPath), MediaType.parse("video/mpeg4")));*/
        builder.addFormDataPart("video", compressVideoPath.substring(compressVideoPath.lastIndexOf("/")),
                RequestBody.create(new File(compressVideoPath), MediaType.parse("video/mpeg4")));
        ...
        Request request = new Request.Builder().put(body)
                .url(UrlConstant.HTTP_PREFIX).build();
        Log.d("tiktok", "myShortPublish: -------body------" + body);
    ...
    private void shortPublish() {
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, final Response response) throws IOException { // 请求成功
                String resp = response.body().string();
                Log.d(TAG, "onResponse: -------shortPublish-----");
                CommitResponse commitResponse = new Gson().fromJson(resp, CommitResponse.class);
                // 回到主线程操纵界面
                runOnUiThread(() -> {
                    mDialog.dismiss(); // 关闭进度对话框
